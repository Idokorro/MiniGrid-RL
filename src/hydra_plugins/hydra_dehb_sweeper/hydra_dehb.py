...
class HydraDEHB(dehb.DEHB):
    def __init__(
        ...
        **kwargs,
        ...
        assert min_fidelity is not None, "Please set a minimum fidelity per run"
        assert max_fidelity is not None, "Please set a maximum fidelity per run"
        super().__init__(
            cs=cs,
            f=f,
    ...
    def _is_run_fidelity_exhausted(self, fevals=None, brackets=None, total_cost=None, total_time_cost=None):
        ...
        if delim_sum == 0:
            raise ValueError("Need one of 'fevals', 'brackets' or 'total_cost' as fidelity for DEHB to run.")
        if fevals is not None:
    ...
    def run(
        ...
        **kwargs,
        ...
        while True:
            if self._is_run_fidelity_exhausted(fevals, num_brackets, total_cost, total_time_cost):
                break
            bracket = None
            ...
            while len(overrides) < jobs_left:
                ...
                new_fidelity = bracket.get_next_job_fidelity()
                if new_fidelity is None:
                    break
                fidelity = new_fidelity
                if reset_job_count:
                    fidelity_left = (total_cost - self.current_total_steps) // fidelity
                    jobs_left = min(jobs_left, fidelity_left)
                    if jobs_left == 0:
                        break

                # Add more jobs if current bracket isn't waiting on results and also has jobs left
                space_in_bracket = (
                    bracket.sh_bracket[fidelity] > 0 and not bracket.previous_rung_waits() and bracket.is_pending()
                )
                while space_in_bracket and len(overrides) < jobs_left:
                    vconfig, _, parent_id = self._acquire_config(bracket, fidelity)
                    ...
                    # log.info(f'Got config {config}, parent id {parent_id} and fidelity {int(fidelity)+1}')
                    names = list(config.keys()) + [self.budget_variable]
                    values = list(config.values()) + [int(fidelity) + 1]
                    if self.slurm:
                        names += ["hydra.launcher.timeout_min"]
                        optimized_timeout = (
                            self.slurm_timeout * 1 / (self.max_fidelity // fidelity) + 0.1 * self.slurm_timeout
                        )
                    ...
                    if self.seeds:
                        ...
                        overrides.append(job_overrides)
                    self.current_total_steps += fidelity
                    bracket.register_job(fidelity)
                    space_in_bracket = bracket.sh_bracket[fidelity] > 0
            ...
            while len(overrides) > 0:
                ...
                if verbose:
                    log.info(
                        "Launching configurations with fidelity {} in bracket {}".format(
                            int(fidelity) + 1, bracket.bracket_id
                        )
                    )
                ...
                for i in range(len(launching_jobs)):
                    launching_jobs[i]["cost"] = launching_jobs[i]["fidelity"]
                done = False
                while not done:
